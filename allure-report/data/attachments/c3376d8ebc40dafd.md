...
- Name: PlaywrightLearning\dataeriven1.spec.js >> search test for monitor
...
```ts
  ...
  15 |         await page.locator("input[value='Search']").click();      // click on the button
> 16 |         await expect.soft(page.locator('h2 a').nth(0)).toContainText(item, { ignoreCase: true });  //  check if results appear
     |                                                        ^ Error: expect(locator).toContainText(expected) failed
  17 |     
  18 |         });
  19 | }
