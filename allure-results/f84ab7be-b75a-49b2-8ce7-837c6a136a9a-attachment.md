...
Call log:
  - waiting for locator('div[class=\'product\']').locator('div[class=\'product\']').filter({ hasText: 'Tomato' }).getByRole('button')
...
```ts
  ...
  36 |       await page.goto('https://rahulshettyacademy.com/seleniumPractise/#/');
  37 |     await page.waitForLoadState('networkidle');
  38 | 
  39 | 
  40 | 
  41 |     const products = page.locator("div[class='product']");
  42 |     console.log(await products.count());
  43 |     await page.screenshot({ path: 'screenshot.png' });
  44 |     const titles = await page.locator("div[class='product'] h4").allTextContents();
  45 |     console.log(titles);
  46 |     
> 47 |     await products.locator("div[class='product']").filter({ hasText: "Tomato" }).getByRole('button').click();
     |                                                                                                      ^ Error: locator.click: Test timeout of 30000ms exceeded.
  48 |     await page.waitForTimeout(2000);
  49 |      await page.screenshot({ path: 'screenshot.png' });
  50 | 
  51 | });
  52 | 
  53 | 
