...
Call log:
  - waiting for locator('div[class=\'product\']').nth(28).locator('text=Tomato - 1 Kg')
    ...
    - generic [ref=e340]:
      ...
      - generic [ref=e345]:
        ...
        - link "+" [ref=e348] [cursor=pointer]:
          - /url: "#"
      - button "ADD TO CART" [active] [ref=e350] [cursor=pointer]
    - generic [ref=e351]:
...
```ts
  ...
  22 | 
  23 |             for(let i=0; i< await products.count();i++){
  24 |                 const productName = await products.nth(i).locator("h4").textContent();
  25 |                 console.log(productName);
  26 | 
  27 |              if(productName.includes("Cashews")){
  28 |                 await products.nth(i).locator("button").click(); // becuase only 1 button is there this will work
> 29 |                 await products.nth(i).locator("text=Tomato - 1 Kg").click(); // becuase only 1 button is there this will work
     |                                                                     ^ Error: locator.click: Test timeout of 30000ms exceeded.
